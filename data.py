    ...
    for action in actions:
        ...
        for sequence in range(no_sequences):
            ...
            for frame_num in range(sequence_length):
                ...
                frame_path = 'Image/{}/{}.png'.format(action, sequence)
                frame = cv2.imread(frame_path)

                #
                # Check if the frame is read properly
                if frame is None:
                    print(f"Error: Unable to read the image from path {frame_path}")
                    continue

                # Make detections using the Mediapipe Hands model
                image, results = mediapipe_detection(frame, hands)
                
                # Draw landmarks on the image using the function `draw_styled_landmarks`
                ...
                # Extract keypoints from the detection results using the function `extract_keypoints`
                keypoints = extract_keypoints(results)
                
                # Define the path to save keypoints in a .npy file
                npy_path = os.path.join(DATA_PATH, action, str(sequence), str(frame_num))
                
                # Save the keypoints as a .npy file using the `np.save` function
